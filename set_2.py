...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    # Get the ASCII value of the letter and shift it
    shifted_ascii = ord(letter) + shift
    
    # Wrap around the alphabet (A=65, Z=90)
...
def vigenere_cipher(message, key):
    ...
    for i in range(len(message)):
        if message[i] == " ":
            ...
            new_letter = chr(((ord(message[i]) - ord('A') + shift) % 26) + ord('A'))
...
def scytale_cipher(message, shift):
    ...
    for i in range(num_columns):
        for j in range(shift):
            encoded_message += message[i + j * num_columns]
...
def scytale_decipher(message, shift):
    ...
    str
        the decoded message
    '''
    # Determine number of columns (rows from encryption)
    num_columns = len(message) // shift
    ...
    for col in range(num_columns):
        for row in range(shift):
            decoded_message[col + row * num_columns] = message[index]
            index += 1
